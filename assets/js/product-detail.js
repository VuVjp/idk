...
function createGameCard(game) {
  ...
  col.className = "col-6 col-md-4 col-lg-3 mb-4"; // Bootstrap grid classes + margin-bottom

  const card = document.createElement("div");
  card.className = "card h-100 shadow-sm rounded-4 overflow-hidden";
  card.style.cursor = "pointer";

  card.innerHTML = `
    <img src="${game.image}" alt="${game.name}" class="card-img-top rounded-top-4" />
    <div class="card-body d-flex flex-column">
      <h5 class="card-title">${game.name}</h5>
      <p class="card-text fw-bold">
        ${game.priceOld && game.priceOld > game.priceNew
          ? `<span style="text-decoration: line-through; color: #888; margin-right: 8px;">${game.priceOld.toLocaleString()} VNĐ</span>`
          : ''}
        ${game.priceNew === 0 ? 'Miễn phí' : `${game.priceNew.toLocaleString()} VNĐ`}
      </p>
      <a href="product-details.html?id=${game.id}" class="btn btn-primary rounded-pill mt-auto buy-now-btn">Mua ngay</a>
    </div>
  `;

  // Khi click vào card bất kỳ, chuyển trang
  card.addEventListener("click", () => {
    window.location.href = `product-details.html?id=${game.id}`;
  });

  // Ngăn nút "Mua ngay" bị sự kiện click của card ghi đè
  const buyNowBtn = card.querySelector('.buy-now-btn');
  buyNowBtn.addEventListener('click', (event) => {
    event.stopPropagation();
    // Link mặc định của <a> vẫn hoạt động bình thường
  });
...
function getRelatedGames(currentId, count = 3) {
  const otherGames = Object.values(products).filter(g => g.id !== currentId);
...
function showRelatedGames() {
  ...
  const relatedGames = getRelatedGames(currentId, 3);
  const container = document.getElementById("related-games");
  container.innerHTML = "";
  relatedGames.forEach(game => {
    const cardElement = createGameCard(game);
